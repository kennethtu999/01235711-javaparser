...
public class BulkNeo4jService {
    ...
    private Map<String, Object> convertClassNodeToMap(Neo4jClassNode node) {
        ...
        map.put("modifiers", node.getModifiers() != null ? node.getModifiers() : List.of());
        map.put("isAbstract", node.getIsAbstract() != null ? node.getIsAbstract() : false);
        map.put("isFinal", node.getIsFinal() != null ? node.getIsFinal() : false);
        map.put("isPublic", node.getIsPublic() != null ? node.getIsPublic() : false);
        map.put("isPrivate", node.getIsPrivate() != null ? node.getIsPrivate() : false);
        map.put("isProtected", node.getIsProtected() != null ? node.getIsProtected() : false);
        map.put("isStatic", node.getIsStatic() != null ? node.getIsStatic() : false);
    ...
    private Map<String, Object> convertMethodNodeToMap(Neo4jMethodNode node) {
        ...
        map.put("isProtected", node.getIsProtected() != null ? node.getIsProtected() : false);
        map.put("isStatic", node.getIsStatic() != null ? node.getIsStatic() : false);
        map.put("isSynchronized", node.getIsSynchronized() != null ? node.getIsSynchronized() : false);
        map.put("isConstructor", node.getIsConstructor() != null ? node.getIsConstructor() : false);
        map.put("isGetter", node.getIsGetter() != null ? node.getIsGetter() : false);
        map.put("isSetter", node.getIsSetter() != null ? node.getIsSetter() : false);
        map.put("sourceFile", node.getSourceFile() != null ? node.getSourceFile() : "");
        map.put("lineNumber", node.getLineNumber() != null ? node.getLineNumber() : 0);
        map.put("columnNumber", node.getColumnNumber() != null ? node.getColumnNumber() : 0);
